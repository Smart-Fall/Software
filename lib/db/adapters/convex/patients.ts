...
export class ConvexPatientRepository implements IPatientRepository {
  ...
  async findById(id: string): Promise<Patient | null> {
    try {
      ...
      return patient
        ? this.mapToPatient(patient as unknown as ConvexPatient)
        : null;
    } catch (error) {
  ...
  async findByUserId(userId: string): Promise<Patient | null> {
    try {
      const patient = await this.client.query(api.patients.getByUserId, {
  ...
  async findMany(options?: FindOptions<Patient>): Promise<Patient[]> {
    try {
      ...
      const patients = await this.client.query(api.patients.list, {
        skip,
        take,
      });
      return (patients as unknown as ConvexPatient[]).map((p) =>
        this.mapToPatient(p),
      );
    } catch (error) {
  ...
  async findByCaregiverId(caregiverId: string): Promise<Patient[]> {
    try {
      ...
      const assignments = await this.client.query(
        api.caregiverPatients.getByCaregiverId,
        {
