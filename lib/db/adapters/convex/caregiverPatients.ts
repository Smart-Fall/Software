...
export class ConvexCaregiverPatientRepository implements ICaregiverPatientRepository {
  ...
  async create(data: {
    ...
    try {
      ...
      const found = (assignment as unknown  as unknown as ConvexCaregiverPatient[]).find(
  ...
  async findByCaregiverId(caregiverId: string): Promise<CaregiverPatient[]> {
    try {
      ...
      return (assignments as unknown  as unknown as ConvexCaregiverPatient[]).map((a) =>
  ...
  async findByIds(caregiverId: string, patientId: string): Promise<CaregiverPatient | null> {
    try {
      ...
      const found = (assignments as unknown  as unknown as ConvexCaregiverPatient[]).find(
